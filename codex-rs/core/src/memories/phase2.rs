...
mod agent {
    ...
    pub(super) fn handle(
        ...
        tokio::spawn(async move {
            ...
            // Check if the agent is already in a final state before subscribing to avoid a race
            // where the agent completes between spawn and subscription.
            let initial_status = agent_control.get_status(thread_id).await;
            if is_final_agent_status(&initial_status) {
                // Agent is already done, handle final status directly
                let final_status = initial_status;
