...
/// Check if text contains sensitive patterns like API keys, tokens, or passwords.
fn contains_sensitive_patterns(text: &str) -> bool {
    // Common API key patterns (simplified regex patterns)
    let sensitive_patterns = [
        r"(?i)sk-[a-zA-Z0-9]{20,}",       // OpenAI API keys
        r"(?i)AKIA[0-9A-Z]{16}",          // AWS access keys
        r"(?i)ya29\.[a-zA-Z0-9_-]{100,}", // Google OAuth tokens
        ...
        r"(?i)ghs_[a-zA-Z0-9]{36}",       // GitHub server tokens
        r"(?i)ghr_[a-zA-Z0-9]{36}",       // GitHub refresh tokens
        r"(?i)xox[baprs]-[0-9]{12}-[0-9]{12}-[0-9]{12}-[a-z0-9]{32}", // Slack tokens
        r"(?i)pk_[a-zA-Z0-9]{32,}",       // Stripe API keys
        r"(?i)Bearer [a-zA-Z0-9_-]{20,}", // Bearer tokens
        r"(?i)password\s*[:=]\s*\S+",     // password: value or password=value
        r"(?i)secret\s*[:=]\s*\S+",       // secret: value or secret=value
        r"(?i)api[_-]?key\s*[:=]\s*\S+",  // api_key: value or api-key=value
    ];
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Check for sensitive patterns (API keys, tokens, passwords) and skip if found.
